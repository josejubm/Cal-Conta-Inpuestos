...
function retornar_vista2($vista, $data = array())
{
    ...
    #DATA DEL FORMULARIO DE EITAR
    $html = str_replace('{CEDULA_HIDEN}', $data['Id'], $html);
    $html = str_replace('{CEDULA}', $data['Id'], $html);
    $html = str_replace('{NOMBRE}', $data['Nombre'], $html);
    $html = str_replace('{APELLIDOS}', $data['Apellidos'], $html);
    $html = str_replace('{CORREO}', $data['Correo'], $html);
    $html = str_replace('{TELEFONO}', $data['Telefono'], $html);
    $html = str_replace('{RFC}', $data['RFC'], $html);
    $html = str_replace('{ESPECIALIDAD}', $data['Especialidad'], $html);
...
function retornar_vista($vista, $data = array())
{
    ...
    foreach ($data['registros'] as $registro) {
        if (!empty($registro['Nombre']) && !empty($registro['Apellidos'])) // Se verifica que el campo Nombre no esté vacío 
        {
            ...
            $fila .= '<td>' . $registro['Apellidos'] . '</td>';
            $fila .= '<td>' . $registro['Correo'] . '</td>';
            $fila .= '<td>' . $registro['Telefono'] . '</td>';
            $fila .= '<td>' . $registro['RFC'] . '</td>';
            $fila .= '<td>' . $registro['Especialidad'] . '</td>';
            $fila .= '<td>' . $registro['Usuario'] . '</td>';
            $fila .= '<td>' . $registro['Titulacion'] . '</td>';
            $fila .= '<td>' . $registro['Registro'] . '</td>';
            $fila .= '<td>' . '<a class="boton boton-outline-warning" href="{EDIT}?cedula=' . $registro['Id'] . '" > <i class="bx bx-edit"></i>Editar</a>' . '</td>';
            $fila .= '<td>' . '<a class="boton boton-outline-danger" href="{DELETE}?cedula=' . $registro['Id'] . '"  > <i class="bx bx-trash"></i>Eliminar </a>' . '</td>';
            $tabla_body .= $fila . '</tr>';
        }
    }
    $tabla_body .= '</tbody>';
